...
2. 收到 `displayState=handoff-ready` 后，使用宿主原生侧边栏导航能力打开返回的短期定向连接 `url`；它只负责把本任务的 workspace/board 绑定到当前标签页，随后重定向到固定、干净、可收藏的本机根地址。普通用户也可以直接访问 `http://127.0.0.1:64775/`，无需先从 Codex 建立连接。已有同一画码标签页时直接复用。不要初始化通用浏览器自动化或 Computer Use。
